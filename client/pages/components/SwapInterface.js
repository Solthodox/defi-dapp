...
export default function DefaultInterface() {
    ...
    const PerformSwap = async(e) => {
        ...
        try{
          ...
          await approval.wait()
          const tx = await pair.swap(selectedTokens[0].address, parseInt(amountIn).toString())
          await tx.wait()
          console.log('Done')
        }catch(e){
          console.log(e)
  ...
  return (
    <div className={styles.container}>
          ...
          <select onChange={e=>getPool(JSON.parse(e.target.value), selectedTokens[1])} className={styles.select}>
              {_tokens.filter(token=>token.name != selectedTokens[1].name && token.name != 'USDC').map((token,i) => 
            ...
            <div className={styles.form}>
              ...
              {_tokens.filter(token=>token.name != selectedTokens[0].name && token.name != 'USDC').map((token,i) => 
