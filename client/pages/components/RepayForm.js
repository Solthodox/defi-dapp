...
import { logos  , symbols , tokenAddresses} from "../../config";
...
export default function BorrowForm(props) {
    ...
    useEffect(()=>{
        if(((props.tokenData[props.selectedSymbol].deposited.value / props.tokenData[0].deposited.amount)*formInput.amount).toFixed(2)*1.25 > (props.collateral.deposited.replace("undefined","")/(10**52)).toFixed(2)) 
            ...
            {!props.processing? (
                <div className='mx-8'>   
                        <div className='flex space-x-2 items-center'>
                            <img  className="h-6  w-6" src={logos[props.selectedSymbol]}></img>
                            <p className='w-full my-4 text-black'>
                                { symbols[props.selectedSymbol]}
                        ...
                        <p className='text-gray-500 my-2 font-semibold'>
                            ...
                            <span className='text-black'>
                                {((props.tokenData[props.selectedSymbol].deposited.value / props.tokenData[0].deposited.amount)*formInput.amount).toFixed(2)} ETH
                            </span> 
                        </p>
                        <p className='text-gray-500 my-2 font-semibold'>
                            Collateral : 
                            <span className='text-black'>
                            {(props.collateral.deposited.replace("undefined","")/(10**52)).toFixed(2)} ETH
                            </span> 
                        </p>
                        {formInput.invalid && (
